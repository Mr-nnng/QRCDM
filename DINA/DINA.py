...
def evaluate_obj(pred, label):
    ...
    acc = metrics.accuracy_score(label, pred.round())
    try:
...
class DINA(CDM):
    ...
    def __init__(self, R, q_m, stu_num, prob_num, know_num, skip_value=-1):
        ...
        self.all_states = initial_all_knowledge_state(
            know_num)  # shape = (state_num, know_num)
        state_prob = np.transpose(
            np.sum(q_m, axis=1, keepdims=True) - np.dot(q_m, np.transpose(self.all_states)))
        # state covers knowledge of problem (1: yes), shape = (state_num, prob_num)
    ...
    def train(self, epoch, epsilon) -> ...:
        ...
        theta, slip, guess, tmp_R = np.copy(self.theta), np.copy(
            self.slip), np.copy(self.guess), np.copy(self.R)
        tmp_R[np.where(self.R == self.skip_value)[0],
              np.where(self.R == self.skip_value)[1]] = 0
        for iteration in tqdm(range(epoch), "training"):
            post_tmp, slip_tmp, guess_tmp = np.copy(
                post), np.copy(slip), np.copy(guess)
            ...
            for s in range(self.state_num):
                log_like = np.log(answer_right[s, :] + 1e-9) * self.R + np.log(1 - answer_right[s, :] + 1e-9) * (
                    1 - self.R)
                log_like[np.where(self.R == self.skip_value)[
                    0], np.where(self.R == self.skip_value)[1]] = 0
            ...
            # shape = (state_num, prob_num)
            r_jl = np.dot(np.transpose(post), tmp_R)
            r_jl_0, r_jl_1 = np.sum(
                r_jl * (1 - self.eta), axis=0), np.sum(r_jl * self.eta, axis=0)
            i_jl_0, i_jl_1 = np.sum(
                i_l * (1 - self.eta), axis=0), np.sum(i_l * self.eta, axis=0)
    ...
    def eval(self, test_data, sub_prob_index) -> tuple:
        pred_score = (1 - self.slip) * self.eta + self.guess * (1 - self.eta)
        if len(sub_prob_index) > 0:
            sub_pred_list, sub_true_list = [], []
        pred_list, true_list = [], []
        for i in tqdm(test_data, "evaluating"):
            stu, test_id, true_score = i['user_id'], i['item_id'], i['score']
            if len(sub_prob_index) > 0 and test_id in list(sub_prob_index):
                sub_pred_list.append(pred_score[self.theta[stu], test_id])
                sub_true_list.append(true_score)
            else:
                pred_list.append(pred_score[self.theta[stu], test_id])
                true_list.append(true_score)
        if len(sub_prob_index) > 0:
            return evaluate_obj(pred_list, true_list), evaluate_sub(sub_pred_list, sub_true_list)
        else:
            return evaluate_obj(pred_list, true_list)

    def save(self, filepath):
        with open(filepath, 'wb') as file:
            pickle.dump({"theta": self.theta, "slip": self.slip,
                        "guess": self.guess}, file)
    ...
    def transform(self, records):  # MLE for evaluating student's state
        ...
        log_like = records * \
            np.log(answer_right + 1e-9) + (1 - records) * \
            np.log(1 - answer_right + 1e-9)
