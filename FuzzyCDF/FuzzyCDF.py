...
def init_parameters(stu_num, prob_num, know_num, args):  # initialize FuzzyCDF parameters
    a = stats.lognorm.rvs(s=args.sig_a, loc=0, scale=np.exp(
        args.mu_a), size=(stu_num, know_num))
    b = stats.norm.rvs(loc=args.mu_b, scale=args.sig_b,
                       size=(stu_num, know_num))
    slip = stats.beta.rvs(a=1, b=2, size=prob_num) * \
        (args.max_s - args.min_s) + args.min_s
    guess = stats.beta.rvs(a=1, b=2, size=prob_num) * \
        (args.max_g - args.min_g) + args.min_g
    theta = stats.norm.rvs(
        loc=args.mu_theta, scale=args.sig_theta, size=stu_num)
...
class FuzzyCDF(CDM):
    ...
    def train(self, epoch, burnin) -> ...:
        A, B, slip, guess = np.copy(self.a), np.copy(
            self.b), np.copy(self.slip), np.copy(self.guess)
    ...
    def eval(self, test_data) -> tuple:
        _, pred_mastery = cal_alpha_mastery(self.a, self.b, self.theta, self.q_m, self.obj_prob_index,
                                            self.sub_prob_index)
        pred_score = (1 - self.slip) * pred_mastery + \
            self.guess * (1 - pred_mastery)
        if len(self.sub_prob_index) > 0:
            sub_pred_list, sub_true_list = [], []
        pred_list, true_list = [], []
        for i in tqdm(test_data, "evaluating"):
            stu, test_id, true_score = i['user_id'], i['item_id'], i['score']
            if len(self.sub_prob_index) > 0 and test_id in list(self.sub_prob_index):
                sub_pred_list.append(pred_score[stu, test_id])
                sub_true_list.append(true_score)
            else:
                pred_list.append(pred_score[stu, test_id])
    ...
    def save(self, filepath):
        with open(filepath, 'wb') as file:
            pickle.dump({"a": self.a, "b": self.b, "theta": self.theta,
                        "slip": self.slip, "guess": self.guess}, file)
            logging.info("save parameters to %s" % filepath)

    def load(self, filepath):
        with open(filepath, 'rb') as file:
            self.a, self.b, self.theta, self.slip, self.guess = pickle.load(
                file).values()
